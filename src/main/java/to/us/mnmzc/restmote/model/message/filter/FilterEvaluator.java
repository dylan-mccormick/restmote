...
/**
 * Evaluates a filter against a set of message attributes. It checks if all conditions of the filter
 * are satisfied by the provided attributes.
 */
public final class FilterEvaluator {
  ...
  public static boolean evaluate(Filter filter, Map<String, Object> attributes) {
    return filter.getConditions().stream().allMatch(condition -> condition.evaluate(attributes));
  }
}
