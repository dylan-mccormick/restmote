...
public class MessageService {
    ...
    public void routeMessage(Message message) {
        ...
        List<Bridge> filteredBridges = correspondingBridges.stream().filter(bridge -> bridge.getFilter() == null || FilterEvaluator.evaluate(bridge.getFilter(), message.getAttributes())).toList();
        log.trace("After filtering, {} corresponding bridges remain.", filteredBridges.size());

        // message attributes -> receiver filters
        List<ReceiverSession> initiallyFilteredReceivers = filteredBridges.stream().flatMap(b -> receiverRegistry.getSessionsForBridge(b.getId()).stream().filter(r -> r.getFilter() == null || FilterEvaluator.evaluate(r.getFilter(), message.getAttributes()))).toList();
        log.trace("After filtering, {} corresponding receivers remain.", initiallyFilteredReceivers.size());
