...
/**
 * The transmitter service keeps track of all the connected transmitters and is responsible for
 * validating transmitters when a connection is requested.
 */
@Slf4j
@Service
public class TransmitterService {
  ...
  /**
   * Initializes the transmitter service with an empty list of transmitters. The list of
   * transmitters will be populated as transmitters are registered.
   */
  ...
  public void register(Transmitter transmitter) {
    if (transmitters.stream().anyMatch(t -> t.getId().equalsIgnoreCase(transmitter.getId()))) {
      log.warn(
          "Attempted to register transmitter with id {}, but it already exists",
          transmitter.getId());
      throw new IllegalStateException(
          "Transmitter with id " + transmitter.getId() + " already exists");
    }
  ...
  /**
   * Unregisters the transmitter with the given id. The transmitter must be registered for it to be
   * unregistered.
   *
   * @param id The id of the transmitter to unregister. The transmitter must be registered for it to
   *     be unregistered.
  ...
  /**
   * Gets the transmitter with the given ID. The transmitter must be registered for it to be
   * returned.
   *
   * @param id The ID of the transmitter to get.
   * @return An Optional containing the transmitter with the given ID if it is registered, or an
   *     empty Optional if it is not registered.
   */
