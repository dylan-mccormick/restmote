...
/**
 * This service is responsible for managing bridges. Holds a list of bridges, and also returns the
 * bridge which matches the provided bridge token.
 */
@Service
@Slf4j
public class BridgeService {
  ...
  /**
   * Gets the bridge with the given auth token. This is used to determine which bridge a message
   * should be routed to.
   *
   * @param ids The list of bridge IDs to get.
   * @return The list of bridges with the given IDs, or an empty list if no bridge matches the given
   *     IDs.
   */
  ...
  /**
   * Initializes the bridge service with the provided configuration. The configuration is used to
   * generate the list of bridges.
   *
   ...
   * @return The bridge with the given auth token, or null if no bridge matches the given auth
   *     token.
   */
  private Bridge toBridge(BridgeConfigEntry entry) {
    return Bridge.builder()
        .id(entry.getId())
        .name(entry.getName())
        // .authToken(entry.getAuthToken()) (future)
        .filter(entry.getFilter() == null ? null : toFilter(entry.getFilter()))
        .build();
  }

  /**
   * Converts a filter configuration entry to a filter. This is used to generate the filter for a
   * bridge from the application properties.
   *
  ...
  /**
   * Converts a filter condition configuration entry to a filter condition. This is used to generate
   * the filter conditions for a bridge from the application properties.
   *
