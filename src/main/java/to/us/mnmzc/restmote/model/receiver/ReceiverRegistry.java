...
public class ReceiverRegistry {
  ...
  public void register(ReceiverSession session) {
    ...
    if (sessions.stream().anyMatch(s -> s.getId().equals(session.getId()))) {
      log.warn(
          "Attempted to register session with ID {}, but it is already registered",
          session.getId());
      throw new IllegalStateException(
          "Session with ID " + session.getId() + " is already registered");
    }
  ...
  /**
   * Unregisters a receiver session. This should be called when a receiver disconnects from the
   * server.
   *
  ...
  /**
   * Gets the sessions for a given bridge. This should be used to determine which receivers should
   * receive a message for a given bridge.
   *
  ...
  public List<ReceiverSession> getSessionsForBridge(String bridgeId) {
    return sessions.stream()
        .filter(s -> s.getBridgeIds().stream().anyMatch(b -> b.equals(bridgeId)))
        .toList();
  }
}
