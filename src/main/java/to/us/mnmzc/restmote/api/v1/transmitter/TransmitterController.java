...
public class TransmitterController {
  ...
  /**
   * Attempts to parse the attribute to another type, then returns a string if it cannot be parsed.
   * Currently, supports parsing to: - Boolean - Number (Integer, Double) - String (if it cannot be
   * parsed to any other type)
   *
   ...
   * @return a Map<String, Object> of the attributes, with values parsed to their appropriate types
   *     where possible
   */
            ...
            Collectors.toMap(
                ...
                e ->
                    e.getValue().size() == 1
                        ...
                        : e.getValue().stream()
                            .map(this::tryParseAttribute)
                            .toList() // if only one value, return the value instead of the list
                ));
  }

  /**
   * Creates a new transmitter with the given name, id, and bridge IDs. If no ID is given, a random
   * one will be generated. The transmitter will also be registered.
   *
   ...
   */
  @PostMapping("/link")
  public ResponseEntity<?> linkTransmitter(
      @RequestParam(required = false) String id,
      @RequestParam String name,
      @RequestParam List<String> bridgeIds) {
    Transmitter.TransmitterBuilder builder = Transmitter.builder().name(name).bridgeIds(bridgeIds);
  ...
  public ResponseEntity<ApiMessage> sendMessage(
      @RequestParam String transmitterId,
      @RequestParam MessagePayloadType payloadType,
    ...
    Transmitter transmitter =
        transmitterService
            .getTransmitterById(transmitterId)
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Transmitter with id " + transmitterId + " not found"));
