...
public class WebSocketReceiverHandler extends TextWebSocketHandler {
  ...
  /**
   * Parses the query parameters of the WebSocket URI to extract any relevant information for the
   * receiver session.
   *
  ...
  /**
   * Ensures "attributes" is either: 1) a JSON object node, or 2) a JSON string that parses into an
   * object node.
   *
   * @param attributesNode the JsonNode representing the "attributes" field in the incoming
   *     WebSocket message
   * @return a Map<String, Object> parsed from the "attributes" field
   * @throws JacksonException if the "attributes" string cannot be parsed into a valid JSON object
   *     or if the JSON object cannot be converted into a Map<String, Object>
   */
  private Map<String, Object> extractAttributesMap(JsonNode attributesNode)
    ...
    if (attributesNode.isObject()) {
      ...
      throw new IllegalArgumentException(
          "'attributes' must be a JSON object or a JSON string containing an object");
    }
  ...
  /**
   * Ensures "filter" is either: 1) a JSON object node, or 2) a JSON string that parses into an
   * object node.
   *
   * @param filterNode the JsonNode representing the "filter" field in the incoming WebSocket
   *     message
   * @return a Filter object parsed from the "filter" field
   * @throws JacksonException if the "filter" string cannot be parsed into a valid JSON object or if
   *     the JSON object cannot be converted into a Filter instance
   */
  private Filter extractFilter(JsonNode filterNode) throws JacksonException {
    ...
    if (filterNode.isObject()) {
      ...
      throw new IllegalArgumentException(
          "'filter' must be a JSON object or a JSON string containing an object");
    }
  ...
  public void handleTextMessage(WebSocketSession session, TextMessage message) {
            ...
            .orElseThrow(
                () ->
                    new IllegalStateException(
                        "Received message for unregistered WebSocket session: " + session.getId()));
    JsonNode json;
    ...
    try {
      ...
      if (!json.has("filter") && !json.has("attributes")) {
        throw new IllegalArgumentException(
            "WebSocket message must contain either 'filter' or 'attributes' field");
      }
  ...
  public void afterConnectionEstablished(WebSocketSession session) throws IOException {
    ...
    ReceiverSession.ReceiverSessionBuilder builder =
        ReceiverSession.builder().id(session.getId()).strategy(strategy);
