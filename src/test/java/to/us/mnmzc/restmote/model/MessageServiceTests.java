...
public class MessageServiceTests {
  ...
  private Message message(
      Transmitter source, Map<String, Object> attributes, Filter receiverFilter) {
  ...
  private ReceiverSession receiver(
      String id,
  ...
  void routeMessage_bridgeFilterRejects_noReceiverLookupOrDelivery() {
    ...
    try (MockedStatic<FilterEvaluator> mocked = Mockito.mockStatic(FilterEvaluator.class)) {
      mocked
          .when(() -> FilterEvaluator.evaluate(bridgeFilter, msg.getAttributes()))
          .thenReturn(false);
  ...
  void routeMessage_receiverFilterRejects_noDelivery() {
    ...
    ReceiverStrategy strategy = mock(ReceiverStrategy.class);
    ReceiverSession r1 =
        receiver("r1", List.of("bridge-1"), Map.of("name", "alpha"), receiverFilter, strategy);
    ...
    try (MockedStatic<FilterEvaluator> mocked = Mockito.mockStatic(FilterEvaluator.class)) {
      mocked
          .when(() -> FilterEvaluator.evaluate(receiverFilter, msg.getAttributes()))
          .thenReturn(false);
  ...
  void routeMessage_messageReceiverFilter_selectsMatchingReceiversOnly() {
    ...
    try (MockedStatic<FilterEvaluator> mocked = Mockito.mockStatic(FilterEvaluator.class)) {
      mocked
          .when(() -> FilterEvaluator.evaluate(messageReceiverFilter, r1.getAttributes()))
          .thenReturn(true);
      mocked
          .when(() -> FilterEvaluator.evaluate(messageReceiverFilter, r2.getAttributes()))
          .thenReturn(false);
