...
public class ReceiverRegistryTests {
  ...
  ReceiverSession getSession() {
    return ReceiverSession.builder().bridgeIds(List.of("test-bridge")).build();
  }

  ReceiverSession getSessionWithBridge(String bridgeId) {
    return ReceiverSession.builder().bridgeIds(List.of(bridgeId)).build();
  }

  ReceiverSession getSessionWithMultipleBridges(String... bridgeIds) {
    return ReceiverSession.builder().bridgeIds(List.of(bridgeIds)).build();
  }
  ...
  class UnregisterTests {
    ...
    void unregisterNonExistent() {
      ...
      Assertions.assertThrows(
          IllegalStateException.class, () -> registry.unregister(session.getId()));
    }
