    ...
    class Initialization {
        ...
        void initializesAllConfiguredBridgesWithMixedFilters() {
            contextRunner.withPropertyValues(
                ...
                var home = service.getBridges().stream().filter(b -> "home".equals(b.getId())).findFirst().orElseThrow();
                var garage = service.getBridges().stream().filter(b -> "garage".equals(b.getId())).findFirst().orElseThrow();
                var office = service.getBridges().stream().filter(b -> "office".equals(b.getId())).findFirst().orElseThrow();
                ...
                Assertions.assertEquals("topic", garage.getFilter().getConditions().getFirst().field());
                Assertions.assertEquals("garage/open", garage.getFilter().getConditions().getFirst().expected());
                ...
                Assertions.assertEquals("office/", office.getFilter().getConditions().get(0).expected());
                Assertions.assertEquals("payload", office.getFilter().getConditions().get(1).field());
                Assertions.assertEquals("ignore", office.getFilter().getConditions().get(1).expected());
            });
        }
    }
    ...
    class GetBridgesByIds {
        ...
        void returnsMatchesInConfiguredOrder() {
            withBaseBridges().run(context -> {
                ...
                Assertions.assertEquals(List.of("home", "office"), result.stream().map(b -> b.getId()).toList());
            });
        }
    }
}
