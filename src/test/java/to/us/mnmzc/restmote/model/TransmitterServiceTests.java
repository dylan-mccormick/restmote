...
public class TransmitterServiceTests {
  ...
  private Transmitter createTransmitter(String id, String name) {
    return Transmitter.builder().id(id).name(name).build();
  }
  ...
  class Register {
    ...
    @Test
    @DisplayName("registers a transmitter with a unique id")
    void registersUniqueId() {
    ...
    @Test
    @DisplayName("registers multiple transmitters when ids are unique")
    void registersMultipleUniqueIds() {
    ...
    @Test
    @DisplayName("rejects registration when transmitter id already exists")
    void rejectsDuplicateId() {
      transmitterService.register(createTransmitter("id-1", "alpha"));

      Assertions.assertThrows(
          IllegalStateException.class,
          () -> transmitterService.register(createTransmitter("id-1", "different-name")));
    }

    @Test
    @DisplayName("allows reusing an id after previous transmitter is unregistered")
    void allowsIdReuseAfterUnregister() {
      transmitterService.register(createTransmitter("id-1", "alpha"));
      transmitterService.unregister("id-1");

      Assertions.assertDoesNotThrow(
          () -> transmitterService.register(createTransmitter("id-1", "beta")));
      Assertions.assertEquals(1, transmitterService.getTransmitters().size());
    }
    ...
    void supportsManyUniqueRegistrations() {
      IntStream.range(0, 100)
          .forEach(i -> transmitterService.register(createTransmitter("id-" + i, "name-" + i)));

      Assertions.assertEquals(100, transmitterService.getTransmitters().size());
    }

    @Test
    @DisplayName("id comparison is case-insensitive")
    void idComparisonIsCaseInsensitive() {
      transmitterService.register(createTransmitter("ID-1", "alpha"));
  ...
  class Unregister {
    ...
    @Test
    @DisplayName("unregisters a previously registered transmitter by id")
    void unregistersRegisteredId() {
    ...
    @Test
    @DisplayName("throws when unregistering an id that is not registered")
    void throwsWhenIdNotRegistered() {
      Assertions.assertThrows(
          IllegalStateException.class, () -> transmitterService.unregister("missing-id"));
    }
    ...
    void throwsWhenUnregisteringFromEmptyService() {
      Assertions.assertThrows(
          IllegalStateException.class, () -> transmitterService.unregister("id-1"));
    }

    @Test
    @DisplayName("id comparison is case-insensitive during unregister")
    void unregisterIdComparisonIsCaseInsensitive() {
      transmitterService.register(createTransmitter("ID-1", "alpha"));
